...
class RewardLTL:
    ...
    def parse_action(states):
        ...
        '''
        parsed_states = []
        preset = ["play_next_page", "wave_hands", "move_head", "ask_question", "Dont_care"]

        for num in states:
            assert num < len(preset), 'Only 4 possible actions!'
            parsed_states.append({preset[int(num)]: True})

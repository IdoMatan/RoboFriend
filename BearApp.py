...
def start_story(story):
    ...
    print(" [x] Sent %r:%r" % ('app', message))


def playpause():
    ''' pausing and playing video '''
    print(play_pause.get())
...
button_run = ttk.Button(buttons_frame, textvariable=play_pause, command=playpause, width=10)

# button_run = ttk.Button(buttons_frame, textvariable=play_pause, command=playpause, bg='dark green', fg='black', relief='raised',
...
def check_queue():
    ...
    if method is not None:
        ...
        if message['manual']:
            ...
            print('Page ended --> Auto mode (algo chooses action)')
    window.after(500, check_queue)
